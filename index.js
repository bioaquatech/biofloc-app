...
exports.loginUser = functions.https.onCall(async (data, context) => {
  // DIAGNOSTIC TEMPORAIRE — à retirer une fois le 401 résolu.
  console.log("loginUser — diagnostic auth:", {
    hasAuth: !!context.auth,
    uid: context.auth && context.auth.uid,
    tokenIss: context.auth && context.auth.token && context.auth.token.iss,
    tokenAud: context.auth && context.auth.token && context.auth.token.aud,
    rawRequestHasAuthHeader: !!(context.rawRequest && context.rawRequest.headers && context.rawRequest.headers.authorization),
  });
  if (!context.auth) {
